...
Setup a chizzl client to receive bounced-down traffic and forward it to the webserver running on the Pi:
...
# ⬇️ Connects to chizzl relay server example.com
...
services:
  chizzl:
    image: jpillora/chizzl
    restart: unless-stopped
    container_name: chizzl
...
services:
  chizzl:
    ...
    command:
      ...
      # ⬇️ Reverse tunnel traffic from the chizzl server to the web server container, identified in Docker using DNS by its service name `webserver`.
      - 'R:80:webserver:80'
    networks:
      - internal
